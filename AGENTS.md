...
- **Theme tagging**: Use ordered broad themes only. The first entry is the primary theme; add one or two secondary themes only when clearly justified by the description or repeated publication pattern. Keep at most 3 themes. Do not use a theme just because a method or one-off publication touches it.
- **Research region tagging**: Prefer the most informative geographic scale. Do not store `Gran Chaco` as a research-region label; the whole directory already implies Chaco relevance. Use countries, national Chaco labels, ecoregions, provinces, or local sites instead. Drop redundant parent labels such as `Argentina` when a more specific region already captures the scope. Default to at most 3 regions.
- **Region filter behavior**: The browse-page `Research Region` filter now matches hierarchically for country and national-Chaco labels. A profile tagged with `Formosa` or `Chaco province` should match `Argentina` without needing `Argentina` duplicated in `geographical_focus`.
...
- **279 researchers, 1562 publications** (as of 2026-05-05)
